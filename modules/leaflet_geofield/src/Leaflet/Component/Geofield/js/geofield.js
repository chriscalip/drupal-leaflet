  ...
  init: function(data) {
    ...
    if (goog.isDef(data.opt) && goog.isDef(data.opt.initialData) && data.opt.initialData) {
      try {
        ...
        var feature = format.readFeature(jQuery('.leaflet-geofield-data', geofieldWrapper).val());
...
// replace this function by what you need
    function saveData() {
      ...
      try {
        ...
        jQuery('.leaflet-geofield-data', geofieldWrapper).val(e.name + ": " + e.message);
        return;
      }
      jQuery('.leaflet-geofield-data').val(datas);
      // @TODO Doesn't look like the code below is needed anymore.
      //if (typeFormat === 'GeoJSON') {
      //  // format is JSON
      //  jQuery('.leaflet-geofield-data', geofieldWrapper).val(JSON.stringify(datas, null, 4));
      //}
      //if (typeFormat === 'GPX' || typeFormat === 'KML') {
      //  // format is XML (GPX or KML)
      //  var serializer = new XMLSerializer();
      //  jQuery('.leaflet-geofield-data', geofieldWrapper).val(serializer.serializeToString(datas));
      //}
      //if (typeFormat === 'WKT') {
      //  jQuery('.leaflet-geofield-data').val(datas);
      //}
...
// clears the map and the output of the data
    function clearMap() {
      ...
      jQuery('.leaflet-geofield-data', geofieldWrapper).val('');
    }
